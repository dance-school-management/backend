...
import { IProductWithProfileServer } from "../../../proto/ProductCommunication_grpc_pb";
import { InstructorData, InstructorIdsRequest, InstructorsDataResponse } from "../../../proto/ProductCommunication_pb";
...
export const ProductWithProfileServiceImpl: IProductWithProfileServer = {
  getOtherInstructorsData: async function (
    call: ServerUnaryCall<InstructorIdsRequest, InstructorsDataResponse>,
    callback: sendUnaryData<InstructorsDataResponse>,
  ): Promise<void> {
    const instructorIds = call.request.getInstructoridList()
    try {
      ...
      response.setInstructorsdataList(instructors.map((instructor) => {
